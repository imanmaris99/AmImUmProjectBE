...
def validate_signature_key(order_id, status_code, gross_amount, server_key, signature_key):
    """
    Memvalidasi signature_key dari Midtrans untuk memastikan integritas data.

    Args:
        order_id (str): ID pesanan.
        status_code (str): Kode status transaksi.
        gross_amount (str): Jumlah total transaksi.
        server_key (str): Server key dari Midtrans.
        signature_key (str): Signature key yang diterima.
...
def handle_notification(
    ...
    try:
        ...
        if not validate_signature_key(
            ...
            midtrans_data.get("status_code"),
            midtrans_data.get("gross_amount"),
            MIDTRANS_SERVER_KEY,
            midtrans_data.get("signature_key"),
        ):
            logger.warning("Signature key tidak valid.")
            return build(error=HTTPException(status_code=400, detail="Signature key tidak valid."))
